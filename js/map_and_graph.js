var xh = window.innerWidth || e.clientWidth || g.clientWidth;
var    yh = window.innerHeight|| e.clientHeight|| g.clientHeight;

console.log('HOLLAAAA', xh, yh);
...
var svg = d3.select("#bigcontainer").append("svg").attr("id", 'containerd3')
	...
	legendvalues = [0.2,0.4,0.6,0.8,1]
	...
	var legendtext= legend.selectAll("key").data(legendvalues).enter().append("svg:text") // text
			...
			.attr("font-size","15pt")
			.attr("fill",'white')
	        .text(function(d){return d*500 +'K'})
   
   var legendtext_title= legend.append("svg:text") // text
	        .attr("x",position_legend_color_x-10)
			.attr("y",function(d,i){return position_legend_color_y+heightlegend-(7-1)*heightlegend/(nlegend-1)})
			.attr("font-size","15pt")
			.attr("fill",'white')
	        .text('Price of a')

   var legendtext_title2= legend.append("svg:text") // text
	        .attr("x",position_legend_color_x-10)
			.attr("y",function(d,i){return position_legend_color_y+heightlegend-(6-1)*heightlegend/(nlegend-1)})
			.attr("font-size","15pt")
			.attr("fill",'white')
	        .text('2Bd Apartment')

   var indstructions= legend.append("svg:text") // text
	        .attr("x",width -140)
			.attr("y",30)
			.attr("font-size","12pt")
			.attr("fill",'white')
			.style("text-align", "right") 
            .append('svg:tspan').attr('x', width -350).attr('dy', 5).text('Instructions:')
            .append('svg:tspan').attr('x', width -350).attr('dy', 25).text('# Go over the map to highlight zip code areas')
            .append('svg:tspan').attr('x', width -350).attr('dy', 25).text('# Scroll the slider to change year')
            .append('svg:tspan').attr('x', width -350).attr('dy', 25).text('# Tick the checkbox to add crimes to the map')
...
//queue().defer(d3.json, "json/Chicago_small.json")
   .await(ready);
	...
	svg.append("svg:text").attr("id","legendtext").attr("x",450).attr("y",70).text("CHICAGO").attr("font-size","30pt").style("fill",colors(0.1)).attr("opacity",0)
;
  ...
  click = !click  
   ...
   if (click == false){
	...
	tooltip.transition()
	   ...
	   .attr("r",6)
	   .style("opacity", 1)
	    tooltip.html("Zipcode "+ d.properties.zip +"<br><small>click to select</small>")
